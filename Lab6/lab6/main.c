...
void* Malloc(unsigned int size)
{
	...
	while (i < MASK_SIZE)
	{
		...
		if (i == MASK_SIZE && sn > 0) return NULL;
		if (sn > 0) continue;
		else
		{
...
void Free(void* p)
{
	...
	POINTERS[pIndex].p = NULL;					//Зануляем указатель
...
//Дефрагментация памяти, принимает переменное число указателей на указатели, последним должен идти NULL.
void defragmentation(void** ppFirst, ...)
{
	void*** ptrs = &ppFirst;											//Массив указателей на указатели в программе
	if (ptrs == NULL) return;										//Если массив пуст, то завершение программы
	for (int i = 0, j = 0; i < MASK_SIZE; i++)							//Проверка на то, были ли указатели посланы верные указатели
	{
		if (POINTERS[i].p == NULL) continue;
		for (j = 0; ptrs[j] != NULL && (*ptrs[j] != POINTERS[i].p); j++);
		if (ptrs[j] == NULL) return;
	}
	...
	while (1)
	{
		...
		for (; free_seg_end < MASK_SIZE; free_seg_start++, free_seg_end++)	//Переписываем маску и указатели
		{
			...
			POINTERS[free_seg_end].p = NULL;
			int i = 0;
			for (; ptrs[i] != NULL; i++)								//Поиск указателя в массиве указателей на указатели
			{
				if (POINTERS[free_seg_start].p == *ptrs[i])
					break;
			}
			if (ptrs[i] == NULL) continue;							//Если не нашли такого указателя, то конец цикл заново
...
int main()
{
	...
	setlocale(LC_ALL, "Rus");
	{
		...
		show_mem();
		puts("Дефрагментация:");
		defragmentation((void**)&str1, (void**)&str2, NULL);
		show_mem();
	...
	show_mem();
	defragmentation((void**)&str1, (void**)&str2, (void**)&str3, NULL);
	show_mem();
	system("pause");
	return 0;
}